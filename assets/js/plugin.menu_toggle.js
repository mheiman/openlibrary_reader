...
*/
  ...
  const toggleRouter = function toggleRouter (br, e, atBookCenter) {
    ...
    if (isValidClickArea) {
      ...
      if (atBookCenter) {
        e.stopImmediatePropagation(); // don't turn the page. this takes prescendence
        e.preventDefault();
      }
    }
  }
...
/**
* Override the the default function to force resizing book pages when
* the navigation toggles
*/
BookReader.prototype.setNavigationView = function brSetNavigationView(hide) {
  ...
  var resizePageContainer = function resizePageContainer () {
    ...
    // Locate and attach the messagebar if present
    if (!this.refs.$BRmessageBar) {
        this.refs.$BRmessageBar = $(document).find("#IABookReaderMessageWrapper");
    }
...
/* Attach a handler to the load expiration event to reshow the toolbars when a loan expires */
var respondToLoanExpire = function(event) {
    console.log("IABookReader:BrowsingHasExpired");
    BookReader.prototype.setNavigationView(false);
};

$("document").on("IABookReader:BrowsingHasExpired", respondToLoanExpire);
